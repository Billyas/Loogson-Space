...
u32_t sys_now(void)
{
    return (1000/CONFIG_TICK_RATE_HZ)*xTaskGetTickCount();
}
...
u32_t sys_arch_mbox_fetch(sys_mbox_t *mbox, void **msg, u32_t timeout)
{
    ...
    if (timeout)
    {
        ucTimeout = (timeout * OS_TICKS_PER_SEC)/1000;
        if (ucTimeout < 1)
            ucTimeout = 1;
        else if (ucTimeout > 65535)
            ucTimeout = 65535;
    }
    else
    {
    ...
    temp = OSQPend(mbox->pQ, ucTimeout, &ucErr);
...
u32_t sys_arch_sem_wait(sys_sem_t *sem, u32_t timeout)
{
    ...
    OSSemPend(sem->sem, ucTimeout, &ucErr);
    if (ucErr == OS_ERR_TIMEOUT)
    {
        timeout = SYS_ARCH_TIMEOUT;
    }
