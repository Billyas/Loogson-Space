...
#include "ls1b.h"
#include "ls1b_irq.h"
#define LS1x_K0_CACHED 	        LS1B_K0_CACHED
#elif defined(LS1C)
#include "ls1c.h"
#include "ls1c_irq.h"
#define LS1x_K0_CACHED 			LS1C_K0_CACHED
#else
...
#define MII_WR_TIMEOUT_US       50  		    // mii  write timeout
#define MII_RD_TIMEOUT_US       50  		    // mii  read  timeout
#define GDMA_RESET_TIMEOUT_US   200  		    // gdma reset timeout

//***************************************************************************************
// Hardware-specific storage, 驱动专用硬件相关的控制结构
//***************************************************************************************

#define USE_OS_EVENT   1                            /* TODO 任务创建不同步, 造成 event 失败 */
...
static void LS1x_GMAC_init_hw(GMAC_t *pMAC, unsigned char *macAddr)
{
    ...
    for (i=0; i<GDMA_RESET_TIMEOUT_US; i++)
    {
        delay_us(1);
...
#if (defined(LS1x_K0_CACHED) && (GMAC_BUF_CACHED))
