...
#define PWM_TIMER_START(pwm) \
        do { \
            pwm->hwPWM->ctrl = pwm_ctrl_cntr_en | pwm_ctrl_ien | pwm_ctrl_single | \
                               pwm_ctrl_hrc_ien | pwm_ctrl_oe_mask; \
            ls1x_sync(); \
        } while (0)
...
STATIC_DRV int LS1x_PWM_initialize(void *dev, void *arg)
{
    ...
    /**************************************************************************
     * create event. XXX 由调用者创建
     */
#if 0
#if defined(OS_RTTHREAD)
    pwm->event = rt_event_create(pwm->dev_name, 0);
#elif defined(OS_UCOS)
    unsigned char err;
    pwm->event = OSFlagCreate(0, &err);
#elif defined(OS_FREERTOS)
    pwm->event = xEventGroupCreate();
#endif
#endif
