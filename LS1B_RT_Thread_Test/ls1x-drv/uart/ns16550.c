...
/*
 * 只能保存UART_BUF_SIZE-1 个字符.
 *
 * 字节: 0  1  2  3  4  5        ... 64
 *       __ __ xx xx xx xx __ __ ... __
 *             ^           ^
 *             pHead       |
 *                         pTail
 *
 * 如果空: pHead==pTail;
 * 字节数: count==pTail-pHead 或者(环绕): pTail-pHead+64 
 */
...
#define REAL_BUF_SIZE   (UART_BUF_SIZE - 1)

static int enqueue_to_buffer(NS16550_t *pUART, NS16550_buf_t *data, char *buf, int len, bool overWrite)
{
    int i, count;
    bool round = (data->pTail < data->pHead);
    char *pBUFEND = data->buf + REAL_BUF_SIZE;

    /*
     * if data length larger than UART_BUF_SIZE-1, overwrite full buffer
     */
    if ((overWrite) && (len >= REAL_BUF_SIZE))
    {
        memcpy(data->buf, buf + len - REAL_BUF_SIZE, REAL_BUF_SIZE);
        data->pHead = data->buf;
        data->pTail = pBUFEND;
        data->count = REAL_BUF_SIZE;

        return REAL_BUF_SIZE;
    }

    if (!overWrite && (len > REAL_BUF_SIZE - data->count))
        count = REAL_BUF_SIZE - data->count;
    else
        count = len;
    ...
    for (i=0; i<count; i++)
    {
        *data->pTail++ = buf[i];
        ...
        if (data->pTail > pBUFEND)
        {
            data->pTail = data->buf;
            round = true;
        }

        if (data->count < REAL_BUF_SIZE)
            data->count++;

        if (overWrite && round && (data->pTail == data->pHead))
            data->pHead = data->pTail + 1;
    }
...
{
    char *pBUFEND = data->buf + REAL_BUF_SIZE;
    int i, count = len < data->count ? len : data->count;
    
    for (i=0; i<count; i++)
    {
        buf[i] = *data->pHead++;
        
        if (data->pHead > pBUFEND)
            data->pHead = data->buf;
    }
...
static void NS16550_interrupt_process(NS16550_t *pUART)
{
    ...
    do
    {
        ...
        if (status & SP_INTID_RXRDY)
        {
             ...
             */
            enqueue_to_buffer(pUART, &pUART->RxData, buf, i, true);
        }
...
static void NS16550_interrupt_handler(int vector, void *arg)
{
    if (NULL != arg)
    {
        ...
        LS1x_INTC_CLR(pUART->IntrCtrl)  =  pUART->IntrMask;   /* 清中断 */
...
//    DBG_OUT("NS16550 (%s) initialized.\r\n", NS16550_get_device_name(pUART));
...
static int NS16550_write_string_int(NS16550_t *pUART, char *buf, int len)
{
    /* add data to transmit cached buffer
     */
    int sent = enqueue_to_buffer(pUART, &pUART->TxData, buf, len, false);
        
    /* send if cached buffer count is more than SP_FIFO_SIZE
     */
    if ((pUART->TxData.count >= SP_FIFO_SIZE) &&
        (NS16550_get_r(pUART->CtrlPort, NS16550_LINE_STATUS) & SP_LSR_THOLD))
    {
        int i, count = 0;
        char txbuf[SP_FIFO_SIZE];

        /* Dequeue transmitted characters from cached buffer
         */
        count = dequeue_from_buffer(pUART, &pUART->TxData, txbuf, SP_FIFO_SIZE);

        /* write data to transmit buffer */
        for (i=0; i<count; ++i)
        {
            NS16550_set_r(pUART->CtrlPort, NS16550_TRANSMIT_BUFFER, txbuf[i]);
        }
...
//        if (count == 0) count = 1;          /* atleast return 1 bytes */
//    #endif
        return count;
    }
    else
...
//                if (i == 0) i = 1;
//#endif
                return i;                   /* atleast return 1 bytes */
            }
        }
    }
    else                                    /* blocking read */
...
static NS16550_t ls1b_UART4 = 
{
    ...
    .bFlowCtrl = false,             // enable on hardware support
    .ModemCtrl = 0,
    .bIntrrupt = false,             // by user needed
 ...
 */
#if defined(OS_RTTHREAD)
const char *NS16550_get_device_name(void *pUART)
{
