...
{
    LS1x_I2C_bus_t *pIIC = (LS1x_I2C_bus_t *)bus;
    struct tm *t = (struct tm *)buf;    // 参数类型 struct tm *
...
//	DBG_OUT("RX8010: read 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
//		    date[0], date[1], date[2], date[3], date[4], date[5], date[6]);
	...
	t->tm_mon  = bcd2bin(date[RX8010_REG_MONTH - 0x10] & 0x1f) - 1;
	t->tm_year = bcd2bin(date[RX8010_REG_YEAR  - 0x10]);
	t->tm_wday = bcd2bin(date[RX8010_REG_WDAY  - 0x10] & 0x7f);
	
	if (t->tm_year < 70)        /* 年份范围: 1970~2069 */
		t->tm_year += 100;
    t->tm_year += 1900;         /* 写入的时候: (t->tm_year % 100) */
    
//	DBG_OUT("RX8010: date %ds %dm %dh %dmd %dm %dy\n",
...
STATIC_DRV int RX8010_write(void *bus, void *buf, int size, void *arg)
{
	...
	date[RX8010_REG_MONTH - 0x10] = bin2bcd(t->tm_mon + 1);
	date[RX8010_REG_YEAR  - 0x10] = bin2bcd(t->tm_year % 100);  /* 只保留年份的低两位 */
	date[RX8010_REG_WDAY  - 0x10] = bin2bcd(t->tm_wday);

//	DBG_OUT("RX8010: write 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
//		     date[0], date[1], date[2], date[3], date[4], date[5], date[6]);
...
int get_rx8010_time(void *bus, struct tm *dt)
{
    int rt = RX8010_read(bus, (void *)dt, sizeof(struct tm), NULL);
    return rt == sizeof(struct tm) ? 0 : -1;
}

int set_rx8010_time(void *bus, struct tm *dt)
{
