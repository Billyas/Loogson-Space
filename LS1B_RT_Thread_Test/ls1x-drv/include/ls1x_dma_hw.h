...
typedef enum
{
	...
	dma_valid			= bit(2),		// 用户请求将当前DMA操作的相关信息写回到指定的内存地址;
										// 当用户写回DMA操作相关信息后,该位清零.
	dma_dev_num_mask	= 0x03,			// bit 1:0, 选择DMA操作的设备.
	dma_dev_nand		= 0x00,			// nand flash
	dma_dev_ac97_read	= 0x01,			// AC97 read device
	dma_dev_ac97_write	= 0x02,			// AC97 write device
} LS1x_dma_order_t;
...
/*
	...
	desc_daddr_ac97_mono		 = bit(30),		// =0: mono; =1: 2 stereo
