...
static  rt_mutex_t      gui_mutex;			/* mutex for change button active group */
#define LOCK()          do { rt_mutex_take(gui_mutex, RT_WAITING_FOREVER); } while (0);
#define UNLOCK()        do { rt_mutex_release(gui_mutex); } while (0);

#elif defined(OS_UCOS)
static  OS_STK          gui_stack[GUI_STK_SIZE];
static  OS_EVENT       *gui_mutex;
#define LOCK()          do { unsigned char err; if (OSRunning == OS_TRUE) OSMutexPend(gui_mutex, ~0, &err); } while (0);
#define UNLOCK()        do { if (OSRunning == OS_TRUE) OSMutexPost(gui_mutex); } while (0);

#elif defined(OS_FREERTOS)
static  SemaphoreHandle_t gui_mutex;
#define LOCK()          do { xSemaphoreTake(gui_mutex, portMAX_DELAY); } while (0);
#define UNLOCK()        do { xSemaphoreGive(gui_mutex); } while (0);
...
int start_gui_monitor_task(void)
{
    ...
    gui_mutex = rt_mutex_create("guimtx", RT_IPC_FLAG_FIFO);
    if (gui_mutex == NULL)
        return -1;
    ...
    gui_mutex = OSMutexCreate(GUI_MUTEX_PRIO, &err);
    if (gui_mutex == NULL)
        return -1;
