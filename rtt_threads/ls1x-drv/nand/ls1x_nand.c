...
typedef struct
{
	...
	unsigned int dmaIrq;					/* DMA  中断 */
	unsigned int dmaIntCtrl;
	unsigned int dmaIntMask;
#endif
 ...
 */
static char dma_desc_base[sizeof(LS1x_dma_desc_t)] __attribute__((aligned(64)));
...
#if (USE_DMA0_INTERRUPT)
	pNand->dmaIrq     = LS1x_DMA0_IRQ;
	pNand->dmaIntCtrl = LS1x_INTC0_BASE;
	pNand->dmaIntMask = INTC0_DMA0_BIT;
#endif
...
#if (USE_DMA0_INTERRUPT)
	/* 设置边沿触发方式 */
	LS1x_INTC_EDGE(pNand->dmaIntCtrl) |= pNand->dmaIntMask;
	LS1x_INTC_POL(pNand->dmaIntCtrl)  &= ~pNand->dmaIntMask;

	/* 禁止中断 */
    LS1x_INTC_IEN(pNand->dmaIntCtrl) &= ~(pNand->dmaIntMask);
	LS1x_INTC_CLR(pNand->dmaIntCtrl)  = pNand->dmaIntMask;
#else
	/* 禁止中断 */
...
static void LS1x_NAND_dma_start(LS1x_NAND_dev_t *pNand, unsigned int size, int rw)
{
	...
	if (DMA_READ == rw) 	/* READ */
		pNand->dmaDesc->cmd = desc_cmd_int_enable;
	else 					/* WRITE */
		pNand->dmaDesc->cmd = desc_cmd_int_enable | desc_cmd_r_w;

	/* Start DMA control, physical address */
	dma_cmd = K1_TO_PHYS((unsigned int)pNand->dmaDesc) & dma_desc_addr_mask;
	dma_cmd |= dma_start | dma_valid;
...
#if (USE_DMA0_INTERRUPT)
	LS1x_INTC_CLR(pNand->dmaIntCtrl) = pNand->dmaIntMask;			/* 清中断标志 */
#endif
...
static int LS1x_NAND_erase_block(LS1x_NAND_dev_t *pNand, unsigned int blkNum, int checkbad)
{
	...
	{
		printk("Try erase blkNum=0x%08X\r\n", blkNum);
		return -EINVAL;
	}

    if (checkbad && (LS1x_NAND_is_badblock(pNand, blkNum) == true))
	{
...
static int LS1x_NAND_erase_chip(LS1x_NAND_dev_t *pNand)
{
	unsigned int blkNum;
	...
	for (blkNum = 0; blkNum < BLOCKS_OF_CHIP; blkNum++)
	{
		...
		if (0 != LS1x_NAND_erase_block(pNand, blkNum, 0))
		{
		    printk("Erase Nand block %i time-out, fail.\r\n", (int)blkNum);
        	return -ETIMEDOUT;
        }
	}

    printk("Erase Nand done.\r\n");
...
#if (USE_DMA0_INTERRUPT)
static void LS1x_NAND_interrupt_handler(int vector, void *arg)
{
	LS1x_NAND_dev_t *pNand = (LS1x_NAND_dev_t *)arg;

	WRITE_REG32(pNand->dmaCtrl, dma_stop);				/* Stop DMA */

	if (pNand->dmaDesc->cmd & desc_cmd_r_w)				/* According dma-desc-cmd to know r/w done */
	{
		pNand->write_cnt++;
...
STATIC_DRV int LS1x_NAND_initialize(void *dev, void *arg)
{
	 ...
	 */
	ls1x_install_irq_handler(pNand->dmaIrq, LS1x_NAND_interrupt_handler, (void *)dev);
#endif

	pNand->intialized = 1;

    DBG_OUT("NAND controller initialized.\r\n");
...
STATIC_DRV int LS1x_NAND_ioctl(void *dev, int cmd, void *arg)
{
	...
	switch (cmd)
	{
		...
		case IOCTL_NAND_ERASE_BLOCK:
			rt = LS1x_NAND_erase_block(pNand, (unsigned int)arg, 1);
			break;

		case IOCTL_NAND_ERASE_CHIP:
			rt = LS1x_NAND_erase_chip(pNand);
			break;
 ...
 * NAND device.
 */
void *devNAND = &ls1x_NAND; // LS1x_NAND_dev_t *
