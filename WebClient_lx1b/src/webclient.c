                                     ...
                                     const char *url, const char **request)
{
    ...
    *request = path_ptr ? path_ptr : "/";
    ...
    rt_kprintf("host address: %s , port: %s\n", session->host, port_str);
...
static int webclient_connect(struct webclient_session *session, const char *URI)
{
...

    {
        ...
        socket_handle = socket(res->ai_family, SOCK_STREAM, IPPROTO_TCP);
#endif
        //rt_kprintf("connect test, create socket(%d) error.", socket_handle);
        if (socket_handle < 0)
        {
            LOG_E("connect failed, create socket(%d) error.", socket_handle);
            rt_kprintf("connect failed, create socket(%d) error.", socket_handle);
        ...
        if (connect(socket_handle, res->ai_addr, res->ai_addrlen) != 0)
        {
            ...
            //rt_kprintf("connect failed, connect socket(%d) error.", socket_handle);
...
int webclient_handle_response(struct webclient_session *session)
{
    ...
    rt_kprintf("webclient_handle_response function\n");
...
int webclient_get(struct webclient_session *session, const char *URI)
{
    ...
    if (rc != WEBCLIENT_OK)
    {
        ...
        rt_kprintf("connect to webclient server failed.\n");
        return rc;
    }
    ...
    if (rc != WEBCLIENT_OK)
    {
        ...
        rt_kprintf("send header to webclient server failed.\n");
       return rc;
    }
    ...
    rt_kprintf("get position handle response(%d).\n", resp_status);
