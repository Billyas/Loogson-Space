...
int yaffs_startup_and_mount(const char *mount_name)
{
	...
	cfg->bytes_of_page         = BYTES_OF_PAGE;      // 2K bytes
	cfg->pages_of_block        = PAGES_OF_BLOCK;     // 64 pages
	cfg->oobbytes_of_page      = OOBBYTES_OF_PAGE;
	cfg->start_block           = 112;   //  14*1024*1024 == 112*64*2K
	cfg->end_block             = 912;   // 100*1024*1024 == 800*64*2K
    ...
    printk("YAFFS: NAND flash mounted as %s %s.\r\n",
            mount_name, (rt == 0) ? "successful" : "fail");
