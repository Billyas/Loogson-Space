...
#define F_READ(fd, buf, count)   yaffs_read(fd, buf, count)
#define F_SEEK(fd, off, whence)  yaffs_lseek(fd, off, whence)
#define F_CLOSE(fd)              yaffs_close(fd)
#else
#define F_READ(fd, buf, count)   fread(buf, 1, count, fd)
#define F_SEEK(fd, off, whence)  fseek(fd, off, whence)
#define F_CLOSE(fd)              fclose(fd)
#endif
#endif
...
static int fb_showbmp_internal(int x, int y, char *bmpfilename)
{
	...
	/* 读取位图文件头 */
    rt = F_READ(fd, (char *)&FileHead, sizeof(BITMAPFILEHEADER));
	...
	/* 读取位图信息头 */
	rt = F_READ(fd, (char *)&InfoHead, sizeof(BITMAPINFOHEADER));
    ...
    F_SEEK(fd, (int)chartolong(FileHead.cfoffBits, 4), SEEK_SET);
	...
	/* 读全部像素数据 */
    rt = F_READ(fd, bmpbuf, BytesPerLine*ciHeight);
