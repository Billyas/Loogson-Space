					 ...
					 int n_bytes, int write_trhrough)
{
	...
	while (n > 0 && chunk_written >= 0)
	{
		    ...
		    dev->param.inband_tags)
		{
			...
			if (dev->param.n_caches > 0)
			{
				...
				if (cache)
				{
					...
					if (write_trhrough)
					{
						chunk_written = yaffs_wr_data_obj(cache->object, cache->chunk_id,
			...
			else
			{
				...
				chunk_written = yaffs_wr_data_obj(in, chunk, local_buffer, n_writeback, 0);
		...
		else
		{
			...
			chunk_written = yaffs_wr_data_obj(in, chunk, buffer, dev->data_bytes_per_chunk, 0);
			       ...
			       int backward_scanning)
{
	 ...
	 */
	obj = yaffs_find_or_create_by_number(dev, obj_id, YAFFS_OBJECT_TYPE_FILE);
	if (!obj)
		return;
	obj->is_shadowed = 1;
...
int yaffs_guts_initialise(struct yaffs_dev *dev)
{
	...
	{
		/*
		 * FIXME it's a bug when no_tags_ecc=0 ? 2014.7, Bian
		 */
