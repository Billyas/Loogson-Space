...
/*
 * modified by loongide @@ 2021.8
 */
#define RT_KPRINTF_FLOAT   1
...
#define PRINTF_FLOAT_BUFFER_SIZE    32
#define DEFAULT_FLOAT_PRECISION     5
#define PRINTF_MAX_FLOAT            1e9

static char* print_float(char* str,
                         char* end,
                         double value,
                         int precision,
                         int field_width,
                         unsigned int flags)
{
    char buf[PRINTF_FLOAT_BUFFER_SIZE];
    int  i, len  = 0;
    double diff = 0.0;
    ...
    if ((field_width > 0) && (field_width < precision)) // set likely %3.6f?
    {
        field_width = 0;
    }
    ...
    while ((len < PRINTF_FLOAT_BUFFER_SIZE) && (precision > 9))
    {
        buf[len++] = '0';
        precision--;
    }
    ...
    else
    {
        ...
        while (len < PRINTF_FLOAT_BUFFER_SIZE)
        {
            --count;
        ...
        // add extra 0s
        while ((len < PRINTF_FLOAT_BUFFER_SIZE) && (count-- > 0))
        {
            buf[len++] = '0';
        }

        if (len < PRINTF_FLOAT_BUFFER_SIZE)
        {
            // add decimal
            buf[len++] = '.';
        }
    ...
    {
        if (field_width && (negative || (flags & (PLUS | SPACE))))
        {
            field_width--;
        }

        while ((len < field_width) && (len < PRINTF_FLOAT_BUFFER_SIZE))
        {
            buf[len++] = '0';
        }
    }
    ...
    {
        rt_memset(str, ' ', field_width - len);    // fill with space
        str += field_width - len;
    }
    ...
    for (i=0; i<len; i++)
    {
        *str = buf[len-i-1];
        ++str;
        if (((field_width > 0) && (i >= field_width)) || (str >= end))
            break;
    }
